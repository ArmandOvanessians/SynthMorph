...
# Only did this one so I don't have to retrain the model
...
# save the models weights for future uses
...
# Upload stained and unstained numpy images
s_data = np.load(r"C:\Users\Armand Ovanessians\Microscope\ORB\result\stain.npy")
us_data = np.load(r"C:\Users\Armand Ovanessians\Microscope\ORB\result\unstained.npy")


for i in range(len(s_data)):
    s_test = s_data[i]
    us_test = us_data[i]
    # apply a third dimension to meet synthmorphs specificities
    s_t = s_test[:,:,np.newaxis]
    us_t = us_test[:,:,np.newaxis]
    # plot the slices
    moving = conform(s_t)
    fixed = conform(us_t)
